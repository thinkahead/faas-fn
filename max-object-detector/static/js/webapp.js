...
function sendImage(data) {
  ...
  // Perform file upload
  $.ajax({
    url: '/function/max-object-detector/model/predict',
    method: 'post',
    processData: false,
    contentType: false,
    data: data,
...
$(function() {
  ...
  $.get('/function/max-object-detector/model/labels', function(result) {
    $.each(result['labels'], function(i, label) {
      $('#label-icons').append($('<img>', {
        class: 'label-icon',
        id: 'label-icon-' + label.id,
        title: label.name,
        src: '/function/max-object-detector/img/cocoicons/' + label.id + '.jpg',
      }));
    });
